...
import math
import os
settings_file = os.path.join(os.path.expanduser('~'),'.config/ant_config/settings.txt')
class DegenerateCodon:
	...
	* If an degenerate codon is passed to the algorithm;
	...
	def getReport(self):
		...
		temp = dict()
		exec(compile(open(settings_file, "rb").read(), settings_file, 'exec'), temp)
	...
	def find_degenerate(self, AA_list):
		...
		for codon_list in regular_triplets:
			...
			for triplet in possible_triplets:
				...
				if len(offtarget) < best_score or best_score == 1000:
		...
		best_triplet = 1000 #for storing best degenerate triplet
		best_offtarget = 1000 #for storing the off-target AA of the best triplet
		best_score = 1000 #for storing the length of the off-target list
		...
		for triplet in good_triplets:
			...
			if len(Realcodons) < best_score or best_score == 1000:
...
if __name__ == "__main__":
	print(DegenerateCodon(['G','A','C']).getReport())
